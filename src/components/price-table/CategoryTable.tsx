...
export default function CategoryTable({ typeValue, items, isDraft, onEdit, onDelete }: CategoryTableProps) {
  ...
  function renderHeaders() {
    switch (typeConfig?.value) {
      ...
      case 'dc_twin_cabling':
        return <><th className="px-4 py-2 text-left">Cabling Name</th><th className="px-4 py-2 text-right">Size (mm)</th><th className="px-4 py-2 text-right">Price/mm</th></>
      case 'cabling_addons':
  ...
  function renderCells(item: PriceItem) {
    ...
    switch (typeConfig?.value) {
      ...
      case 'dc_twin_cabling': return <><td className="px-4 py-2.5 text-slate-300">{r(d.dc_cabling_name)}</td><td className="px-4 py-2.5 font-mono text-brand-400 text-xs text-right">{r(d.size_twin_dc_cable_mm)}</td><td className="px-4 py-2.5 font-mono text-brand-400 text-xs text-right">{r(d.twin_dc_cable_price_per_mm, '$')}</td></>
